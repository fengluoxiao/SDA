...
async function configureInput(device,layout) {
  if(!device?.available || !/SDA Spatial Bitstream/.test(device.name) || !/^\{0\.0\.0\.00000000\}\.\{[a-f0-9-]{36}\}$/i.test(device.id))throw Error('未找到可配置的 SDA 虚拟输入');
...
async function discoverEndpoints() {
  const result=await new Promise((resolve,reject)=>execFile('powershell.exe',['-NoProfile','-File',path.resolve(__dirname,'../windows-system-audio/configure-endpoints.ps1'),'-DiscoverOnly'],{windowsHide:true,timeout:15000,maxBuffer:65536},(error,stdout,stderr)=>error?reject(Error(String(stderr||error.message))):resolve(stdout)));
