...
export function useRemoteSession(state:RemotePlayback,control:(command:RemoteCommand)=>void|Promise<void>,suspend:(replaceOutput:boolean)=>void|Promise<void>) {
  ...
  const current=useRef({state,control,suspend});current.current={state,control,suspend};
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    const publish=()=>{const {coverUrl,scene,...state}=current.current.state;window.sdaDesktop?.publishRemoteState?.({...state,artwork:cover.current.source===(coverUrl??"")?cover.current.data:""} as RemotePlayback);};
