...
### Virtual HDMI sink identity (0.1.0.12)

This is an audio-only virtual HDMI sink, not a virtual monitor or a graphics HDMI/EDID emulator. Windows already receives an HDMI connector category, connected jack description and HDMI sink-information response from the topology. Version 0.1.0.12 replaces the sample sink identity with distinct `SDA HDMI (System / Remote)` and `SDA HDMI (Dedicated)` descriptions. It removes sample claims of HDCP and auxiliary HDMI packet support. The topology bridge now advertises only the implemented DD+ / DD+ Atmos encoded formats alongside PCM routing, matching the wave endpoint instead of advertising unsupported DTS/TrueHD sample formats.

For ordinary playback, choose System / Remote (or retain it as Windows default), select **PCM / 自动识别** in SDA, select the real headphones/speakers for SDA output, and start reception. Objects are optional: stereo and multichannel PCM use the existing renderer. Shared-mode Windows converts application audio to the endpoint's 48 kHz mix. Native exclusive PCM remains limited to the advertised 48 kHz formats; this change does not add other sample rates or invent height channels. Players that enumerate only graphics-attached physical HDMI outputs may still exclude this audio-only device.

For exclusive PCM or DD+ passthrough, choose Dedicated explicitly so the System / Remote endpoint stays available for UU loopback. Both endpoints retain their topology reference names to preserve existing bindings across a normal driver update. Desktop feedback prevention recognizes both old Bitstream names and new HDMI names. A driver update can require a user-initiated reboot; changing the visible name alone does not activate new kernel capability declarations.

Relevant Windows contracts: [HDMI jack sink information](https://learn.microsoft.com/en-us/windows-hardware/drivers/audio/ksproperty-jack-sink-info), [KSJACK_SINK_INFORMATION](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ksmedia/ns-ksmedia-_ksjack_sink_information), [IAudioClient::IsFormatSupported](https://learn.microsoft.com/en-us/windows/win32/api/audioclient/nf-audioclient-iaudioclient-isformatsupported).

Installation status (2026-09-18 16:51): version 0.1.0.12 installed as oem173.inf. SetupAPI reported SUCCESS, but PNP_VetoDevice / CR_REMOVE_VETOED prevented the live device restart and Windows explicitly requires a reboot. Existing endpoints still expose the previous driver identity. New HDMI sink properties have not yet been verified on the running kernel; restart Electron alone is insufficient.

Post-reboot verification (2026-09-18): v0.1.0.12 is active and both HDMI endpoints enumerate with distinct names. Windows regenerated endpoint IDs on this update, so existing third-party explicit device bindings may need reselection despite unchanged topology reference strings. System / Remote remains the multimedia default. Live 12-channel 48 kHz PCM submission completed in both shared System / Remote and exclusive Dedicated modes. Simultaneous System / Remote loopback recordings contained nonzero rendered stereo (other mix channels zero); the receiver retained one return stream, and GameViewerServer remained active on the shared endpoint. This confirms the PCM/render/loopback path; the phone's audible output and third-party player compatibility require listening/application checks. SDA was left open in PCM / automatic reception mode with physical Realtek headphones as shared output.

Endpoint preparation now runs at desktop startup and before reception. Discovery excludes inactive endpoint tombstones. Preparation repairs the Dedicated-as-system-default mistake and registry-backed PotPlayer explicit selections that still reference a known old SDA endpoint, or exclusively target System / Remote. Physical devices, generic default selections, and valid shared-mode System / Remote selections remain unchanged. Running PotPlayer settings are not overwritten: startup maintenance retries every ten seconds while a repair is pending, applying it after the player exits. Portable INI-backed players and other applications are not covered by this adapter; Windows has no general API to rewrite a third-party player's saved explicit endpoint or its live exclusive stream.

SDA automatically publishes the selected shared input format and PhysicalSpeakers when reception starts. This removes the separate Windows speaker-configuration step for supported shared layouts. It does not bypass the Windows shared mixer: exact exclusive PCM can negotiate independently; layouts exceeding the tested shared capability still need exclusive discrete input. HDMI identification alone does not force a player to output more channels than its decoder provides.
