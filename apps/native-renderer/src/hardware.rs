...
impl Settings {
    pub fn validate(&self) -> Result<(), String> {
        for (v, lo, hi) in [
            (self.input_db, -60.0, 12.0),
            (self.line_rms, 0.1, 12.0),
            (self.gain_db, 0.0, 40.0),
            (self.rail_v, 1.0, 80.0),
            (self.current_a, 0.01, 30.0),
            (self.load_ohms, 2.0, 600.0),
            (self.output_ohms, 0.0, 20.0),
            (self.bandwidth_hz, 5000.0, 250000.0),
        ] {
            if !v.is_finite() || v < lo || v > hi {
                return Err("invalid hardware parameter".into());
            }
        }
        if !(8..=24).contains(&self.dac_bits) {
            return Err("invalid DAC precision".into());
        }
        Ok(())
    }
}
...
impl Chain {
    pub fn new(s: &Settings) -> Self {
        ...
        for (i, t) in taps.iter_mut().enumerate() {
            ...
            let sinc = if x == 0.0 {
                ...
                (std::f32::consts::PI * 0.225 * x).sin() / (std::f32::consts::PI * x)
            };
        ...
        Self {
            enabled: s.enabled,
            drive: 10.0_f32.powf(s.input_db / 20.0),
            steps: (1u32 << (s.dac_bits - 1)) as f32,
    ...
    pub fn process(&mut self, input: f32) -> f32 {
        ...
        let quantized = ((input * self.drive).clamp(-1.0, 1.0 - 1.0 / self.steps) * self.steps)
            .round()
            / self.steps;
        let mut output = 0.0;
        ...
        for phase in 0..4 {
            ...
            for i in 0..TAPS {
                reconstructed += self.taps[i] * self.up[(self.cursor + TAPS - i) % TAPS];
            }
            self.pole += self.alpha * (reconstructed * self.volts - self.pole);
            self.down[self.cursor] =
                self.pole.clamp(-self.limit, self.limit) * self.divider / self.volts;
            if phase == 0 {
                for i in 0..TAPS {
                    output += self.taps[i] * self.down[(self.cursor + TAPS - i) % TAPS];
                }
            }
...
mod tests {
    use super::*;
    #[test]
    fn ahb2_gain_modes_preserve_normalized_level() {
        for (line, gain) in [(2.0, 23.0), (4.0, 17.0), (9.8, 9.2)] {
            let s = Settings {
                ...
                line_rms: line,
                gain_db: gain,
                rail_v: 40.0 * (1.0 + 1.0 / 254.0),
                current_a: 29.0,
                load_ohms: 8.0,
                output_ohms: 8.0 / 254.0,
            ...
            for n in 0..12000 {
                let y = c.process(
                    0.5 * (2.0 * std::f32::consts::PI * 1000.0 * n as f32 / 48000.0).sin(),
                );
                assert!(y.is_finite());
                if n >= 1000 {
    ...
    #[test]
    fn exact_bypass_and_reset() {
        let mut c = Chain::new(&Settings::default());
        for x in [-2.0, 0.0, 0.123, 2.0] {
    ...
    #[test]
    fn resistive_load_and_current_limit() {
        let mut s = Settings::default();
        s.enabled = true;
        s.input_db = 0.0;
        s.current_a = 0.01;
    ...
    #[test]
    fn low_level_gain_and_invalid_values() {
        let mut s = Settings::default();
        s.enabled = true;
        let mut c = Chain::new(&s);
        let mut energy = 0.0;
        for n in 0..48000 {
            let y =
                c.process(0.01 * (2.0 * std::f32::consts::PI * 1000.0 * n as f32 / 48000.0).sin());
            if n > 1000 {
                energy += y * y;
            }
        }
