...
impl Default for Settings {
    fn default() -> Self {
        Self {
            monitor: crate::monitor::Settings::default(),
            enabled: false,
...
impl Settings {
    pub fn validate(&self) -> Result<(), String> {
        ...
        if !bounded(self.direct_db, -24.0, 6.0)
            || !bounded(self.early_db, -40.0, 6.0)
            || !bounded(self.late_db, -40.0, 6.0)
            || !bounded(self.early_ms, 10.0, 100.0)
            || !bounded(self.crossover_hz, 40.0, 160.0)
            || !bounded(self.bass_db, -24.0, 6.0)
            || self.speakers.len() > 32
        {
            return Err("invalid cinema settings".into());
        }
        for (name, speaker) in &self.speakers {
            let valid_name = name == "LFE"
                || name == "LFE2"
                || vbap::speakers(vbap::LayoutId::Dolby9_1_6)
                    .iter()
                    ...
                    .any(|s| s.name == name);
            if !valid_name
                || !bounded(speaker.gain_db, -24.0, 6.0)
                || !bounded(speaker.delay_ms, 0.0, 20.0)
                || !bounded(speaker.low_db, -6.0, 6.0)
                || !bounded(speaker.high_db, -6.0, 6.0)
            {
                return Err("invalid speaker calibration".into());
            }
            if name == "LFE" && (speaker.low_db != 0.0 || speaker.high_db != 0.0) {
                return Err("LFE calibration supports gain and delay only".into());
            }
        }
        Ok(())
    }
    ...
    pub fn mix(
        ...
        for i in 0..length {
            ...
            let reflection_gain = weight
                * match if self.enabled {
    ...
    pub fn calibrate(
        ...
        let Some(settings) = self.speakers.get(name) else {
            return Ok(pair);
        };
        ...
        let process = |input: Vec<f32>| -> Result<Vec<f32>, String> {
            ...
            let mut output = vec![
                ...
                input.len()
                    + delay
                    + if settings.low_db != 0.0 || settings.high_db != 0.0 {
                        2048
                    } else {
                        0
                    }
            ...
            for i in delay..output.len() {
                output[i] = high.process(low.process(input.get(i - delay).copied().unwrap_or(0.0)))
                    * db(settings.gain_db);
            }
            Ok(output)
        };
...
impl BassSplit {
    pub fn new(frequency: f32) -> Result<Self, String> {
        Ok(Self {
            frequency,
            low: dsp::Lr4Lowpass::new(48000, frequency)?,
...
impl RoomProfile {
    pub fn load(path: &str) -> Result<Self, String> {
        ...
        if meta.len() > 64 * 1024 * 1024 {
            return Err("room profile exceeds 64 MB".into());
        }
        let profile: Self =
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.measurement == "simulated"
            && !self.simulation.as_ref().is_some_and(|v| v.is_object())
        {
        ...
        if self.version != 1
            ...
            || !["personal", "dummy-head", "simulated"].contains(&self.measurement.as_str())
            || self.speakers.len() != expected.len()
        {
        ...
        for speaker in expected {
            let entries: Vec<_> = self
                ...
                .collect();
            if entries.len() != 1 {
                return Err("room requires one response per layout speaker".into());
            }
            let s = entries[0];
            if !s.azimuth.is_finite()
                || !s.elevation.is_finite()
                || (s.azimuth - speaker.azimuth).abs() > 1.0
                || (s.elevation - speaker.elevation).abs() > 1.0
            {
                return Err("room measurement directions do not match the layout".into());
            }
            let length = s.room_left.len();
            if !(512..=32768).contains(&length)
                || s.room_right.len() != length
                || s.direct_left.len() != length
                || s.direct_right.len() != length
                || s.onset_sample >= length
            {
            ...
            for values in [&s.direct_left, &s.direct_right, &s.room_left, &s.room_right] {
                if !values.iter().all(|v| v.is_finite() && v.abs() <= 16.0)
...
mod tests {
    ...
    fn cinema_neutral_mix_and_calibration_preserve_interaural_delay() {
        ...
        let mut dry_r = dry_l.clone();
        dry_l[10] = 1.0;
        dry_r[17] = 0.5;
        let mut room_l = dry_l.clone();
        let mut room_r = dry_r.clone();
        room_l[300] = 0.2;
        room_r[320] = 0.1;
    ...
    fn cinema_crossovers_are_complementary_and_reject_invalid_values() {
        for frequency in [20.0, 80.0, 1000.0] {
            ...
            for i in 0..96000 {
                ...
                if i >= 48000 {
                    energy += (low + high).powi(2);
                }
            }
            assert!(
                (energy / 48000.0 - 0.5).abs() < 0.015,
                "frequency={frequency}"
            );
        }
        let mut settings = Settings::default();
        settings.early_ms = f32::NAN;
    ...
    fn cinema_audition_removes_reflections_without_altering_direct_ir() {
        ...
        let mut settings = Settings {
            enabled: true,
            reflection_mode: ReflectionMode::Direct,
            ..Settings::default()
        };
        assert_eq!(
            settings.mix((&direct, &direct), (&room, &room), 1.0, 128),
    ...
    fn cinema_reflection_controls_leave_direct_sound_intact() {
        let mut dry = vec![0.0; 8192];
        dry[128] = 1.0;
        let mut room = dry.clone();
        room[700] = 0.5;
        room[6000] = 0.25;
