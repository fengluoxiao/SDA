...
impl StereoFifo {
    ...
    pub(super) fn snapshot_from_producer(&self) -> Vec<f32> {
        ...
        let read = self.effective_read(self.read.load(Ordering::Acquire), write);
        let count = write.wrapping_sub(read).min(self.capacity);
        let mut samples = Vec::with_capacity(count * 2);
        for offset in 0..count {
            samples.extend_from_slice(&unsafe {
                *self.frames[read.wrapping_add(offset) % self.capacity]
                    .0
                    .get()
            });
        }
    ...
    fn apply_flush_epoch_from_consumer(&self, epoch: usize) -> bool {
        ...
        self.flush_ack_epoch.store(epoch, Ordering::Release);
...
mod tests {
    ...
    fn producer_snapshot_preserves_unplayed_samples_across_wraparound() {
        ...
        fifo.pop_frames(5, |_, _| {});
        fifo.push(&[13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0, 20.0]);
        let snapshot = fifo.snapshot_from_producer();
        assert_eq!(
            snapshot,
            vec![11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0, 20.0]
        );
        assert_eq!(fifo.available_read(), 5);
        let mut consumed = Vec::new();
        fifo.pop_frames(5, |_, v| consumed.extend_from_slice(&v));
