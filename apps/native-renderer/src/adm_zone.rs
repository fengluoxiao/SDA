...
impl Zone {
    pub(crate) fn valid(&self) -> bool {
        match self {
            Self::Cartesian { min, max } => (0..3).all(|i| {
                min[i].is_finite()
                    && max[i].is_finite()
                    && min[i] >= -1.0
                    && max[i] <= 1.0
    ...
    pub(crate) fn contains(&self, az: f32, el: f32) -> bool {
        match self {
            ...
            Self::Polar { min, max } => {
                ...
                let in_az = el.abs() >= 90.0 - EPS
                    || span >= 360.0 - EPS
                    || offset <= span.rem_euclid(360.0) + EPS
                    || offset >= 360.0 - EPS;
...
pub(crate) fn apply(
    gains: &mut [f32; vbap::MAX_BUS_COUNT],
    ...
    if excluded[..n].iter().all(|v| *v) || excluded[..n].iter().all(|v| !*v) {
        return;
    }
    ...
    for from in 0..n {
        ...
        for to in 0..n {
            ...
            let b = positions[to];
            let key = [
                LAYERS[layer(elevations[from])][layer(elevations[to])] as f32,
                (sign(a[1]) - sign(b[1])).abs(),
...
mod tests {
    ...
    fn polar_wrap_poles_and_cartesian_bounds() {
        ...
        assert!(rear.contains(180.0, 0.0));
        assert!(rear.contains(-160.0, 0.0));
        assert!(!rear.contains(0.0, 0.0));
        assert!(rear.contains(0.0, 90.0));
        let left = Zone::Cartesian {
            min: [-1.0, -1.0, -1.0],
            max: [-0.1, 1.0, 1.0],
        };
        assert!(left.contains(90.0, 0.0));
        assert!(!left.contains(-90.0, 0.0));
    }
    #[test]
    fn downmix_conserves_energy_and_all_excluded_is_identity() {
        ...
        let original = gains;
        let zone = Zone::Cartesian {
            min: [-1.0, -1.0, -1.0],
            max: [-0.1, 1.0, 1.0],
        };
        apply(&mut gains, &solver, &[zone.clone()]);
        assert!(
            (gains.iter().map(|v| v * v).sum::<f32>()
                - original.iter().map(|v| v * v).sum::<f32>())
            .abs()
                < 1e-5
        );
        for (i, gain) in gains.iter().enumerate().take(solver.bus_count()) {
            let (az, el) = solver.speaker_direction(i);
            if zone.contains(az, el) {
                assert_eq!(*gain, 0.0);
            }
        }
        let before = gains;
