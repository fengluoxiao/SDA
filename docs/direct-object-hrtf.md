...
保持原路由增益，未选中的音箱（含 LFE）降低 24 dB，并在 3D 中降低到 22%
...
同一测试也覆盖聚焦，以未选中路由乘以 `10^(-24/20)` 并单独滤波的参考验证输出。
背景音箱额外使用 1.5 kHz 一阶低通与 25% 原信号混合，降低中高频遮蔽；
选中音箱保持完整频谱和 HRTF。独立对象使用各自的背景卷积历史，保留原布局。
LFE 只降低电平。增益和频谱选择均平滑过渡，不自动补偿响度。
