...
2026-09-10：移除原生与 Web Audio 双耳末端固定 +6 dB 补偿（含原生立体声原音），保留用户音量、监听设置、节目平衡和峰值保护。响度缓存升级为按文件身份隔离的 v2；缺失缓存不再当作 0 LUFS，旧错误缓存不再使用。首次实时测量门槛修正为 57 个重叠块（约 6 秒），目标仍为 −18 LUFS、只衰减。

响度缓存进一步升级为 v3：只在整曲解码完成后写入，丢弃 v2 可能保存的前奏/半曲测量。首次完整播放前持续收敛；重播使用完整测量，暂停/停止不覆盖缓存。渲染器重建保留已确定的节目增益。


立体声 Master 平衡：按 BS.1770-4 对文件已经混好的左右声道计算整曲综合响度，目标 −18 LUFS。两路统一增益，既可衰减也可提升；提升受整曲采样峰值约束，保留 1 dB 余量。它不分别归一化乐器/声道，也不把用户音量或房间/耳机处理纳入歌曲响度测量。此目标不是固定耳机声压，也不保证经过房间及 HRTF 后的信号仍恰为 −18 LUFS。最终双耳峰值保护继续有效（采样峰值保护，不是真峰值测量）。

v4 缓存仅保存完整解码的综合响度与峰值；首次播放使用累积测量平滑收敛，完整播放后的重播使用固定整曲增益。旧缓存不用于提升。带 dialnorm 的内容保持原元数据规则。静音不会被放大。

范围更新：音量平衡仅作用于解码后的纯 L/R 双声道曲目，使用 Master 测量，不套用 dialnorm 增益。多声道、对象音轨及 ADM 母版始终旁路，容器的双声道核心标记或输出选择为立体声不构成启用依据。开关偏好保留，换回纯立体声时自动生效。
